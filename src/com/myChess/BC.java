...
public class BC extends ChessPiece implements MouseListener{
	...
	public void init() {	
		...
		//去掉边框
		this.setBorder(null);
		ImageIcon icon = new ImageIcon("src\\images\\BC.GIF");
		this.setIcon(icon);
	}
	...
	public void setIconSelect(Boolean tf) {
		...
		if(tf == true) {
			ImageIcon icon = new ImageIcon("src\\images\\BCS.GIF");
			this.setIcon(icon);
		}
	}
	...
	public void mouseClicked(MouseEvent var1) {
		...
		if (c==MouseEvent.BUTTON1 && Data.BorRRun == false)// 这个算是正常的棋子选择时的操作，就是换Icon之类的操作，BorRRun表示该哪一方走了
		{
			if(Data.lastSelectPiece!=this && Data.lastSelectPiece!=null) {//如果连续两次点了不同的棋子，就会将上次点的棋子设为未选中，这次点的棋子设为选中
				...
				if(Data.BorRRun == false)//但setLocation执行则满足规则，执行时会将BorRRun取反，所以BorRRun与开始相反时就说明子被吃了，
