...
public class RB extends ChessPiece implements MouseListener{
	...
	public void init() {	
		...
		//去掉边框
		this.setBorder(null);
		ImageIcon icon = new ImageIcon("src\\images\\RB.GIF");
		this.setIcon(icon);
	}
	...
	public void setIconSelect(Boolean tf) {
		...
		if(tf == true) {
			ImageIcon icon = new ImageIcon("src\\images\\RBS.GIF");
			this.setIcon(icon);
		}
	}
	...
	public void setLocation(int setsimX, int setsimY) {
		...
		if(setsimY>4 && Data.panelArray[(CurrentsimX+setsimX)/2][(CurrentsimY+setsimY)/2]==0) {//别象脚和不能过河			
			if(setsimX==CurrentsimX-2&&setsimY==CurrentsimY+2 || setsimX==CurrentsimX+2&&setsimY==CurrentsimY+2
