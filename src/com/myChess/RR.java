...
public class RR extends ChessPiece implements MouseListener{
	...
	public void init() {	
		...
		//去掉边框
		this.setBorder(null);
		ImageIcon icon = new ImageIcon("src\\images\\RR.GIF");
		this.setIcon(icon);
	}
	...
	public void setIconSelect(Boolean tf) {
		...
		if(tf == true) {
			ImageIcon icon = new ImageIcon("src\\images\\RRS.GIF");
			this.setIcon(icon);
		}
	}
	...
	public void setLocation(int setsimX, int setsimY) {
		// TODO Auto-generated method stub
			...
			int BRflagX = 0;
			int BRflagY = 0;
			if(setsimX < CurrentsimX)//这下面一大堆都是对车的走子规则进行限定
